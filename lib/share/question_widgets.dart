...
Color backgroudButtom = Colors.white;
...
void submitLogic(BuildContext context, int questionIndex, int listQuestionIndex, String questionType) {
  if (questionIndex < cobjectList[0].questions.length && questionType != 'TXT') {
    ...
    if (questionIndex == 0) {
      ...
      Navigator.of(context)
          .pushNamed(TextQuestion.routeName, arguments: ScreenArguments(cobjectList, indexTextQuestion, 'TXT', listQuestionIndex));
    }
  }else {
  ...
  return Align(
    child: ButtonTheme(
      ...
      child: MaterialButton(
        ...
        onPressed: () => submitLogic(context, questionIndex, listQuestionIndex, questionType),
      ),
    ),
  );
}
